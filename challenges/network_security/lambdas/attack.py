...
def sendReq(server='http://2.59.200.0', timeout=2):
    return curl("curl --connect-timeout {} {}".format(timeout, server).split(' '))

def sendAttack(server='http://2.59.200.0', timeout=2):
...
def curl(cmd):
    ...
    try:
        ...
        if errcode is None:  # No error code means we were able to connect
            ...
            return errcode.group(1) != "28"  # errcode 28 means connection timeout
...
def handler(event, context):
    msg = ""
    victim = os.environ.get("VICTIM", "http://2.59.200.0")
    # req = sendReq(victim)
